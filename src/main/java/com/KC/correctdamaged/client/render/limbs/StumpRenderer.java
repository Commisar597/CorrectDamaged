...
public class StumpRenderer {
    ...
    public void render(
        ...
        player.getCapability(LimbManager.LIMB_DATA_CAP).ifPresent(data -> {
            ...
            renderArmStumps(poseStack, buffer, packedLight, player, parentModel, parentModel.rightArm,
                    rightArm, isSlim, 0, StumpTextureResolver.LimbType.RIGHT_ARM);
            renderArmStumps(poseStack, buffer, packedLight, player, parentModel, parentModel.leftArm,
                    leftArm, isSlim, 1, StumpTextureResolver.LimbType.LEFT_ARM);

            renderLegStumps(poseStack, buffer, packedLight, player, parentModel, parentModel.rightLeg,
                    rightLeg, 2, StumpTextureResolver.LimbType.RIGHT_LEG);
            renderLegStumps(poseStack, buffer, packedLight, player, parentModel, parentModel.leftLeg,
    ...
    private void renderArmStumps(
            ...
            PlayerModel<AbstractClientPlayer> parentModel, ModelPart parentLimb, ArmData arm,
            boolean isSlim, int limbId, StumpTextureResolver.LimbType limbType
    ) {
        ...
        if (!s) {
            renderCapAtPosition(poseStack, buffer, packedLight, player, parentModel, parentLimb,
        ...
        float centerX = isSlim ? (limbId == 0 ? -0.5F : 0.5F) : 0.0F; // Внимание: центрирование 0.0F для не-slim
    ...
    private void renderLegStumps(
            PoseStack poseStack, MultiBufferSource buffer, int packedLight, AbstractClientPlayer player,
            PlayerModel<AbstractClientPlayer> parentModel, ModelPart parentLimb, LegData leg,
            int limbId, StumpTextureResolver.LimbType limbType
    ) {
    ...
    private void renderCapAtPosition(
            PoseStack poseStack, MultiBufferSource buffer, int packedLight, AbstractClientPlayer player,
            PlayerModel<AbstractClientPlayer> parentModel, ModelPart parentLimb, ModelPart capModel,
            String baseTexName, float yOffset, float centerX, int limbId, boolean isSlim,
            StumpTextureResolver.LimbType limbType, boolean isRoot
    ) {
        poseStack.pushPose();
