...
public class OctreeMeshSplitter {
    ...
    public static CubeUV getOctantUV(CubeUV fullUV, int octantIndex) {
        ...
        // ИСПРАВЛЕНО: Для левого поля (правое ухо 0..8) развёртка идёт спереди назад от 8 к 0.
        // Поэтому задняя часть (iZ = true) должна считывать начало текстуры (offsetX = false).
        FreeUVCubeRenderer.FaceUV left   = subUV(fullUV.left(),  !iZ,  iY);

        // ИСПРАВЛЕНО: Для правого поля (левое ухо 16..24) развёртка идёт спереди назад от 16 к 24.
        // Поэтому задняя часть (iZ = true) считывает конец текстуры (offsetX = true).
